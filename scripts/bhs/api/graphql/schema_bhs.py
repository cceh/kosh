...
def extract_sense(entry_tei):
    ...
    entry = tree.xpath('.')[0]
    bhs_sense = entry.xpath('./ns:sense', namespaces=namespaces)[0]
    print()
    bhs_sense = etree.tostring(bhs_sense, encoding='unicode', pretty_print=True)
    soup = BeautifulSoup(bhs_sense, 'lxml')
    bhs_sense = soup.get_text()
    # remove \n
    bhs_sense = bhs_sense.replace('\n', '')
    return bhs_sense
...
class BHSEntry(graphene.ObjectType):
    id = graphene.String()
...
class BHSQuery(graphene.ObjectType):
    entries = graphene.List(BHSEntry, query=graphene.String(), query_type=graphene.String(),
                            field=graphene.String(),
                            size=graphene.Int())

    ids = graphene.List(BHSEntry, lemma_id=graphene.List(graphene.String), size=graphene.Int())
