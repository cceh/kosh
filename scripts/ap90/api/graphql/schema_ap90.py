...
def extract_sense(entry_tei):
    ...
    entry = tree.xpath('.')[0]
    ap90_sense = entry.xpath('./ns:sense', namespaces=namespaces)[0]
    ap90_sense = etree.tostring(ap90_sense, encoding='unicode', pretty_print=True)
    soup = BeautifulSoup(ap90_sense, 'lxml')
    ap90_sense = soup.get_text()
    # remove \n
    ap90_sense = ap90_sense.replace('\n', '')
    return ap90_sense
...
class AP90Entry(graphene.ObjectType):
    id = graphene.String()
...
class AP90Query(graphene.ObjectType):
    entries = graphene.List(AP90Entry, query=graphene.String(), query_type=graphene.String(),
                            field=graphene.String(),
                            size=graphene.Int())

    ids = graphene.List(AP90Entry, lemma_id=graphene.List(graphene.String), size=graphene.Int())
