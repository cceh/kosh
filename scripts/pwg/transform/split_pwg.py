...
def create_tei_file(entries, slice_number):
    ...
    title.text = 'Böhtlingk-Roth Sanskrit-German Dictionary' + '_' + str(slice_number)
...
def split_pwg(pwg_dir):
    parser = etree.XMLParser(recover=True)
    tree = etree.parse(pwg_dir + 'pwg.tei', parser)
    ...
    chunks = [entries[x:x + 2000] for x in range(0, len(entries), 2000)]
...
pwg_dir = '../../../../c-salt_sanskrit_data/sa_de/pwg/'
